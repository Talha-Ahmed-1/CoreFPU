...
#endif
#include <iostream>
class TopFile_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    TopFile_api_t(VTopFile* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_RegOut)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["TopFile.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VTopFile* top = new VTopFile;
    std::string vcdfile = "test_run_dir/Top_File_Test/TopFile.vcd";
    ...
    Verilated::mkdir("test_run_dir/Top_File_Test/logs");
    VerilatedCov::write("test_run_dir/Top_File_Test/logs/coverage.dat");
#endif
    delete top;
    exit(0);
}
